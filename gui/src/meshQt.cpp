...
void MeshQt::labelVerticesInBBOX( double minX, double maxX, double minY, double maxY,int side, double labelValue,bool border, bool onlyBorder,QColor annocolor,QColor bordercolor,double borderThickness=0.5){
    ...
    for( auto const& currVertex: mVertices ) {
        ...
        if(border){
            if(z > minZ && z < maxZ){
                ...
                if(x >= minX && x <= (minX+borderThickness) && y > minY && y < maxY){
                    this->mRenderColors->setColorSettings(MeshGLColors::COLOR_LABEL_SOLID,bordercolor);
                    glPaint();
                    result.insert(currVertex);
                    //cout<<"MinX Add: "<<std::to_string(x)<<" "+std::to_string(y)<<" "+std::to_string(z)<<"\n";
                    //cout<<"MinX Add Comp: "<<std::to_string(minX)<<" "+std::to_string(maxX)<<" "+std::to_string(minY)<<" "<<std::to_string(maxY)<<"\n";
